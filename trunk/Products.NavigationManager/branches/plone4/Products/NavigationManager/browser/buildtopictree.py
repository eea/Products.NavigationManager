...
def buildTopicTree(context, obj=None, strategy=NavtreeStrategyBase()):
    ...
    def insertElement(context, itemPaths, item,
                      forceInsert=False, topicItem=True):
        ...
        itemInserted = (
            itemPaths.get(itemPath, {}).get('item', None) is not None)
        ...
        parentPruned = (
            itemPaths.get(parentPath, {}).get('_pruneSubtree', False))
        ...
        if insert:
            ...
            if strategy.showAllParents and isCurrentParent:
                # If we will be expanding this later,
                # we can't prune off children now
                expand = True
            else:
    ...
    if strategy.showAllParents and objPath is not None:
        ...
        if len(parentPaths) > 0:
            ...
            for r in results:
                insertElement(context, itemPaths, r,
                              forceInsert=True, topicItem=False)
